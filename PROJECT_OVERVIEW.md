...
- [ ] **Phase 1** — Core shorten + redirect API (base62 ID gen, Postgres via Prisma, Redis cache)
...
├── internal/
│   ├── handlers/             ← HTTP handlers (shorten, redirect, etc.)
│   ├── redis/                 ← Redis client + cache/counter/rate-limit logic
│   ├── db/                    ← Prisma-generated client wrapper
│   ├── middleware/            ← auth, rate limiting, logging
│   └── config/                ← env/config loading
├── prisma/
│   └── schema.prisma          ← DB schema (source of truth for Postgres)
├── docker-compose.yml
├── go.mod
└── .env.example
```
