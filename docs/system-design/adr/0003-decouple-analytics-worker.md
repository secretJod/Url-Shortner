...
3. **Replace the custom Redis Stream worker with a managed queue product.** Rejected outright — every candidate is either paid or adds an external dependency with no clear benefit over the already-implemented, already-correct Redis Streams consumer-group pattern; violates CLAUDE.md §20/§25 (no blind architecture change without evidence of need) since the current mechanism is not shown to be broken, only under-scaled operationally.
