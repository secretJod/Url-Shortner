...
## Status
✅ **Accepted and implemented.** `POST /api/keys` now only sends a magic-link verification email (Option 2, as decided below); `GET /api/keys/verify?token=...` issues the API key after the one-time token is consumed. Email is sent via `internal/mail` (SMTP), backed by MailHog in local/dev. See `05-security.md` SEC-01 (resolved) and `02-high-level-design.md`'s auth flow diagram.
